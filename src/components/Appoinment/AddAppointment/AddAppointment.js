...
import AddAppoinmentCard from '../AddAppoinmentCard/AddAppoinmentCard';
    ...
    return (
        <div>
            <div className="container">
                <h1 className="text-brand my-4">Available Appointment On {date.toDateString()}</h1>
                <div className="row">
                    {
                        AppointmentData.map(info => <AddAppoinmentCard date={date} appInfo={info} key={info._id}></AddAppoinmentCard>)
                    }
                </div>
            </div>
        </div>
